...
const Dashboard: React.FC<DashboardProps> = ({ accounts, incomes, notes, setNotes }) => {
  ...
  const stats = [
    { label: 'Entradas', val: totalIncome, icon: TrendingUp, color: '#10b981', trend: '+12%' },
    { label: 'Saídas', val: totalExpense, icon: TrendingDown, color: '#f43f5e', trend: '-2%' },
    { label: 'Líquido', val: balance, icon: Wallet, color: '#6366f1', trend: 'Estável' },
  ];

  const generateTips = async () => {
    setIsGeneratingTips(true);
    const result = await queryAIAgent("Dê 3 dicas curtas de investimento para meu saldo.", { accounts, incomes });
  ...
  return (
    <div className="space-y-6 md:space-y-10 animate-in fade-in duration-1000 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <h2 className="text-3xl md:text-4xl font-black tracking-tighter text-theme-main">Seu Panorama 2026</h2>
        </div>
        <button onClick={() => setShowExportMenu(!showExportMenu)} className="w-full md:w-auto btn-modern flex items-center justify-center gap-2 shadow-lg">
          <Download size={18} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-7 space-y-4">
            <div className="flex items-center gap-3 px-2">
              ...
              <h4 className="font-black uppercase text-xs tracking-[0.2em] text-theme-main">Próximos Vencimentos</h4>
            </div>
            <div className="grid grid-cols-1 gap-3">
              {upcomingBills.map((bill) => {
                ...
                return (
                  <div key={bill.id} className={`p-4 md:p-5 rounded-3xl bento-card border-white/5 flex items-center justify-between group transition-all ${isUrgent ? 'bg-rose-500/[0.03] border-rose-500/10' : ''}`}>
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-2xl ${isUrgent ? 'bg-rose-500/10 text-rose-400' : 'bg-white/5 text-gray-400'}`}>
                        <Clock size={18} />
                      </div>
                      <div className="max-w-[120px] md:max-w-none">
                        <p className="text-sm font-bold text-theme-main truncate">{bill.description}</p>
                        <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">{bill.bank}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-base md:text-lg font-black text-theme-main">R$ {bill.value.toLocaleString('pt-BR')}</p>
                      <span className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${isUrgent ? 'bg-rose-500/20 text-rose-400' : 'bg-indigo-500/10 text-indigo-400'}`}>
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="space-y-3">
               {aiTips.length > 0 ? aiTips.slice(0, 3).map((tip, idx) => (
                ...
                <div className="p-8 md:p-10 bento-card border-dashed flex items-center justify-center opacity-40 text-[10px] font-black uppercase tracking-widest text-center">Analise sua carteira com o Oráculo</div>
               )}
            </div>
          </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 pt-6">
        {stats.map((s, i) => (
          <div key={i} className="bento-card p-6 md:p-8 group overflow-hidden relative border-white/10">
            <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><s.icon size={80} /></div>
            <div className="flex items-center gap-4 mb-4">
              <div className="p-3 bg-white/5 rounded-2xl border border-white/5 group-hover:border-indigo-500/50 transition-all"><s.icon size={20} style={{ color: s.color }} /></div>
            ...
            <h3 className="text-2xl md:text-3xl font-black tracking-tight text-theme-main">R$ {s.val.toLocaleString('pt-BR')}</h3>
          </div>
        ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto md:h-[400px]">
        <div className="lg:col-span-8 bento-card p-6 md:p-8 min-h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={[{ name: 'Jan', val: totalIncome || 1 }, { name: 'Atual', val: balance }]}>
        ...
        <div className="lg:col-span-4 bento-card p-6 md:p-8 min-h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={statusData.length > 0 ? statusData : [{name: 'Vazio', value: 1}]} cx="50%" cy="50%" innerRadius={60} outerRadius={80} dataKey="value" stroke="none">
